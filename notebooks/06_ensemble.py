...
for ax, m, color in zip(axes[:n_models], list(frames.keys()), palette):
    ...
    ax.set_title(
        ...
        f'MAE={r_row.MAE:.3f}°C  MAPE={r_row.MAPE:.2f}%  R²={r_row.R2:.4f}',
...
ax_ens.set_title(
    ...
    f'MAE={ens_r.MAE:.3f}°C  MAPE={ens_r.MAPE:.2f}%  R²={ens_r.R2:.4f}',
    fontsize=10, fontweight='bold'
)
...
print()
print(f"  {'Model':<28}  {'RMSE':>8}  {'MAE':>8}  {'MAPE':>8}  {'R²':>8}")
print(f"  {'─'*66}")
for _, row in comparison.iterrows():
    marker = ' ←' if row.model == 'ENSEMBLE (weighted avg)' else \
             ' ★' if row.RMSE == comparison.RMSE.min() else ''
    print(f"  {row.model:<28}  {row.RMSE:>8.4f}  {row.MAE:>8.4f}  {row.MAPE:>7.2f}%  {row.R2:>8.4f}{marker}")
print()
print("  Outputs:")
