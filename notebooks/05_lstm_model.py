...
# Inverse-transform predictions and actuals back to °C
def inverse_target(scaled_vals, scaler, n_features):
    """Inverse transform only the target column (index 0) from scaled space."""
    dummy = np.zeros((len(scaled_vals), n_features), dtype=np.float32)
    dummy[:, 0] = scaled_vals
    return scaler.inverse_transform(dummy)[:, 0]

y_pred_actual = inverse_target(y_pred_scaled, scaler, n_features)
y_true_actual = inverse_target(y_test_w,      scaler, n_features)

rmse = np.sqrt(mean_squared_error(y_true_actual, y_pred_actual))
mae  = mean_absolute_error(y_true_actual, y_pred_actual)
mape = np.mean(np.abs((y_true_actual - y_pred_actual) / (np.abs(y_true_actual) + 1e-8))) * 100
...
lstm_metrics = {'model': 'LSTM', 'RMSE': rmse, 'MAE': mae, 'MAPE': mape, 'R2': r2}

# Save test predictions CSV for ensemble
os.makedirs('data/predictions', exist_ok=True)
test_dates_for_pred = combined_df[DATE_COL].iloc[len(train_df):].reset_index(drop=True)
lstm_pred_df = pd.DataFrame({
    'date'      : test_dates_for_pred.values[:len(y_pred_actual)],
    'id'        : range(len(y_pred_actual)),
    'prediction': y_pred_actual,
    'actual'    : y_true_actual
})
lstm_pred_df.to_csv('data/predictions/lstm.csv', index=False)
print(f"\n  Saved -> data/predictions/lstm.csv  ({len(lstm_pred_df)} rows)")
